...
(function($) {
	...
	$.fn.fileUploader = function(config){
        ...
        var startUploadOnesMore = function(){
			$("#px_display").delay(1000).fadeOut("slow",function(){
				$("#px_display").html("");
				$("#pxupload_form").html("");
				itr = 0;
				$limit = 1;
		...
		$.fileUploader.change = function(e){
			...
			var display = '<div class="uploadData" id="pxupload'+ itr +'_text" title="pxupload'+ itr +'">' + 
				...
				'<div class="status">В очереди...</div></div>';
			
            $("#px_display").hide();
			$("#px_display").append(display);
            $("#px_display").fadeIn("slow");
		...
		$(config.buttonUpload).click(function(){
			if (itr > 1){
				...
				$("#pxupload_form form").each(function(){
					...
					if (input_val != ""){
						$(id + "_text .status").text("Загружается...");
						...
						$(id +"_frame").load(function(){
                            $('#uploaded_images').append('<img src="'+$(this).contents().text()+'"/>');
							...
							//}else{
							$(id + "_text").css("background-color", "cfdad0");
							//	up_output = config.errorOutput;
							//}
							up_output += '<br />' + '<b>Загрузка завершилась.</b>'; //$(this).contents().find("#message").text();
                            $(id + "_text").parent().hide();
							$(id + "_text .status").html(up_output);
                            $(id + "_text").parent().fadeIn("slow");
							$(e).remove();
		...
		$(config.buttonClear).click(startUploadOnesMore);
		...
		var px = {
			...
			appendForm: function(){
				...
				var contents = '<form class="upload-form" method="post" id="'+ formId +'" action="'+ px.formAction +'" enctype="multipart/form-data" target="'+ iframeId +'">' +
